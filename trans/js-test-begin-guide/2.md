...
“预期结果(s)” 可以有几种含义，至少对这本书来说是这样的。 以为例，如前一章**所述，每个**输入的输出应该是正确的; 因为这指的是最终的输出。 还有一种输出，形式为“**视觉输出**”。 例如，对于每一个用户交互或事件，我们的 web 应用程序通常会提供一种视觉提示，让用户知道有什么事情正在发生。 在这种情况下，我们所希望的视觉线索将被视为“**预期结果**”。 【显示】
...
2.  Now, for the HTML container which will contain the debugging messages, enter the following code snippet before`</body>` tag:
    ...
    这里所发生的是，前面的 HTML 元素将用于在调试消息和简单应用程序本身之间提供视觉上的分离。 现在保存文件，加载到你的浏览器，你会看到一个类似的例子下一个截图:
