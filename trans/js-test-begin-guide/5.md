...
> 欢迎来到第五章。 这一章相当简单明了; 我们基本上把[第四章](4.html "Chapter 4. Planning to Test")中讨论的计划付诸实施。
...
在本节中，我们将执行回归测试。 我们将尝试通过编写修复最初应用测试计划时发现的错误的代码，来模拟一种支持回归测试的情况。 在编写完代码之后，我们将首先测试我们编写的代码，然后我们将测试整个应用程序，看看它是否在一致性中工作。
...
我们简要讨论了是否应该编写支持关闭 JavaScript 的用户的应用程序的问题。 虽然有不同的观点关于我们是否应该支持这样的用户,最好的方法之一,依我拙见,我们至少应该告诉用户他们的浏览器不支持 JavaScript(或 JavaScript 是关闭的),他们可能会错过一些东西。 为了实现这一点，我们可以使用以下代码片段:
...
既然我们已经讨论了测试计划的执行，现在该转到稍微困难一点的事情上了——测试更复杂的代码。 注意，我们一直在以一维的方式处理 JavaScript:我们将所有的 JavaScript 和 CSS 一起放在 HTML 文件中。 我们一直在开发 JavaScript 代码，因为这是我们使用的唯一一段 JavaScript 代码。 但实际上，web 应用程序通常使用不止一段 JavaScript 代码; 这些额外的代码通常是通过外部 JavaScript 文件附加的。
