...
前一章重点讨论了*是什么*成分，它们由什么组成，*为什么*。 本章重点介绍 JavaScript 组件之间的粘合——*如何*。 如果我们的组件是为特定目的而设计的，那么它们需要与其他组件通信，以实现更大的行为。 例如，路由组件不太可能更新 DOM 或与 API 通信。 我们有擅长这些任务的组件，所以其他组件可以通过与它们沟通来要求它们执行这些任务。
