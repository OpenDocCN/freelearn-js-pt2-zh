...
在本章中，我们将介绍用于 JavaScript 的**Windows 库**（`WinJS`库）提供的一些功能，Microsoft 引入该库是为了使用 JavaScript 访问 Windows 应用商店应用的 Windows运行时。Windows Library for JavaScript 是一个 CSS 和 JavaScript 文件库。它包含一组功能强大且功能丰富的 JavaScript 对象、函数和方法，这些对象、函数和方法被组织到名称空间中，目的是使开发人员更容易使用 JavaScript 创建 Windows 应用商店应用。
...
所有向 Windows 应用商店应用公开的Windows 运行时 API 都包装在`Promise`对象中，公开返回`Promise`对象的方法和函数，允许您在应用中轻松实现异步进程。
