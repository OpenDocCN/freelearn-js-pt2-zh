...
非编译代码有好有坏。 不需要编译，开发者可以在很短的时间内在一个完整的 web 应用程序上快速构建一个网页。 此外，对于新手或中级开发人员来说，它通常是非常平易近人的，这使 web 项目的人员配备变得更容易一些。
...
在 Windows 上，一个JavaScript 编辑器是专属于 Windows 平台，这实际上是一个编辑器，而不是一个 IDE 是 notepad++。 与 Sublime Text 类似，notepad++主要用作文本编辑器，并且有插件支持，但不使用像 Sublime Text 这样的包管理器，所以即使有插件支持，应用程序也运行得非常快。 它还支持一些项目文件的代码提示，包括 JavaScript。
...
Cloud9编辑器，可用在[http://c9.io/](http://c9.io/)，是一个通用的 web 应用 IDE，但是一个云应用程序，支持 HTML5, PHP, Node.js, Rails, Python/Django 和 WordPress。 以下截图显示了 Cloud9 编辑器的用户界面:
