...
我们通过相应的自定义元素处理模板中的组件，例如 web 组件。不要对相似性感到困惑，React 组件是从浏览器中抽象出来的，而 web 组件的工作方式与浏览器本身类似。React 的核心概念是，虚拟 DOM 允许我们避免不必要的 DOM 回流循环，从而使库更适合于高性能应用程序。React 非常适合使用 Node.js 在服务器上呈现静态页面。因此，我们可以在服务器端和客户端之间重用应用程序组件。
