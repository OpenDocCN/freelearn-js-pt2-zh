...
这将安装Meteor在您的系统。 `curl`是去获取脚本的命令。 [https://install.meteor.com](https://install.meteor.com)是脚本的 URL/位置，`/bin/sh` 当然是运行脚本的脚本解释器“Shell”的位置。

一旦你运行了这个脚本，假设你有一个互联网连接和适当的权限，你会看到Meteor包下载和安装:

![Installing with curl](graphics/0823OS_01_08.jpg)

我们在前面的安装文本中寻找的关键是Meteor的位置:
...
这个位置会根据你是在 Linux 还是 Mac OS X 运行而变化，但是它会把 Meteor 放到一个你可以从其他任何地方访问 Meteor 脚本的位置。 这一点很快就会变得很重要。 现在，让我们看看当Meteor安装完成时，我们会得到什么样的友好消息:
...
太棒了! 您已经成功安装Meteor，您正在您的方式创建您的第一个Meteor web 应用!
...
在Meteor的精彩的人已经包括几个示例应用，你可以快速创建和玩，帮助你得到一个更好的想法什么Meteor是有能力的。
...
我们现在可以使用Meteor`create`命令和`--example`参数创建`todos`示例应用的本地副本:
...
与Meteor安装本身一样，`create`命令脚本有一个友好的成功信息:
...
是的,就是这样。 Meteor为我们照顾一切，阅读所有的文件和脚本，并设置 HTTP 侦听器:
...
这是Meteor在行动! 它监视对文件的任何更改，当它看到文件更改时，它会告诉浏览器已经进行了更改，并且它应该刷新自己以获得最新版本。
