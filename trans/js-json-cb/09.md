...
使用jsonpathwithjavascript 在您的 web 应用是容易的。 您只需要在应用中包含`jsonpath.js`实现，然后使用它的`jsonPath`函数。
...
在您的 PHP 应用需要使用JSONPath你包括 JSONPath PHP 实现[可以在 https://code.google.com/p/jsonpath/](https://code.google.com/p/jsonpath/),和解析 JSON 字符串一个 PHP 混合对象在应用 JSONPath 路径之前你想从`jsonPath`函数中提取数据。
