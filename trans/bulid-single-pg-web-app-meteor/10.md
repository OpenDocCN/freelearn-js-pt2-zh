...
如果您只是想将数据放入本地 Meteor 应用程序的数据库，使用`$ meteor`启动 Meteor 服务器，并运行以下命令:
...
如果一切顺利，我们的应用程序应该可以在`<your server's ip>:8080`。
...
目前，Meteor 将本地部署限制在自己的服务器上，对环境的控制有限。 计划中的是一个名为**Galaxy**的企业级服务器基础设施，将使流星应用的部署和扩展像流星本身一样简单。
