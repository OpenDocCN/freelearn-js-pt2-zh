...
为了使在数据库中插入和更新文档成为可能，我们需要设置约束，以便不是每个人都可以更改数据库。 在《流星》中，这是通过允许和拒绝规则完成的。 这些函数将在将文档插入数据库之前检查文档。
...
这将扩展流星默认使用的`underscore`，与额外的字符串函数，如`_.slugify()`，从字符串生成一个 slug。
...
为了使用 allow和 deny 规则启动，我们需要从应用程序中删除`insecure`包，这样没有客户端可以简单地更改数据库而不通过我们的 allow 和 deny 规则。
...
当我们成功移除包后，我们可以使用`meteor`命令再次运行流星。
...
如果 allow规则返回`false`，则拒绝插入文档。 否则，我们将成功添加一个新帖子。 更新的工作方式相同，只是我们只检查当前用户是否是管理员:
...
现在我们有了一个动态的**Save**按钮，让我们将实际的方法添加到我们的应用程序中。为此，我们将直接在`my-meteor-blog`文件夹中创建一个名为`methods.js`的新文件。 通过这种方式，它的代码将被加载到服务器和客户机上，这是作为存根在客户机上执行方法所必需的。
