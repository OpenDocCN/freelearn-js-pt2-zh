...
为了使在数据库中插入和更新文档成为可能，我们需要设置约束，以便不是每个人都可以更改数据库。 在Meteor中，这是通过允许和拒绝规则完成的。 这些函数将在将文档插入数据库之前检查文档。
...
这将扩展Meteor默认使用的`underscore`，与额外的字符串函数，如`_.slugify()`，从字符串生成一个 slug。
...
为了使用 allow和 deny 规则启动，我们需要从应用中删除`insecure`包，这样没有客户端可以简单地更改数据库而不通过我们的 allow 和 deny 规则。
...
当我们成功移除包后，我们可以使用`meteor`命令再次运行Meteor。
