...
为了使用路由器，我们需要设置它。 为了保持代码的组织性，我们将直接在`my-meteor-blog`文件夹的根目录下创建一个名为`routes.js`的文件，其中包含以下代码:
...
**会议。 setDefault('lazyloadLimit'， 2)从**subscriptions.js**文件的**行需要放在`routes.js`文件的开始，在`Router.configure()`函数之前:
