...
3.  Now, we can create the tests by adding a folder called `tests` to our package's folder and create a file called `tests.js` inside.
    ...
    目前，Meteor没有记录`tinytest`包，但它很小，这意味着它是非常简单的。
...
这将启动Meteor应用，并运行我们的包测试。 为了看到它们，我们导航到`http://localhost:3000`:
...
本文摘自[http://velocity.meteor.com](http://velocity.meteor.com)。 此外，Velocity 还添加了Meteor存根和自动存根等功能。 它可以创建用于独立测试的镜像应用并运行安装代码(fixture)。
...
现在，当我们使用`$ meteor`启动Meteor服务器时，我们将看到 Jasmine 包已经在`/my-meteor-blog/tests/jasmine/server/unit`文件夹中创建了两个文件，其中包含了我们包的存根。
...
在这里，我们再次将`home`模板添加到`div`，但这次我们以管理员用户身份登录，使用我们的管理员凭证。 在我们登录后，我们调用`Tracker.afterFlush()`给Meteor时间重新渲染模板，然后检查按钮现在是否存在。

因为这个测试是异步运行的，我们需要调用`done()`函数，我们将它作为参数传递给`it()`函数，告诉 Jasmine 测试已经结束。

### 注释

我们的凭证内的测试文件是安全的，Meteor不捆绑文件在`tests`目录。
...
虽然安装过程不像安装Meteor包那么简单，你需要自己安装并运行 MongoDB 和 PhantomJS，然后才能运行测试。
...
现在你已经读了整本书，我假设你知道更多的Meteor比以前，并对这个框架感到兴奋，因为我是!

如果你有任何关于 Meteor 的问题，你可以在[http://stackoverflow.com](http://stackoverflow.com)上问他们，那里有一个很棒的 Meteor 社区。

我还建议阅读所有Meteor子项目[https://www.meteor.com/projects](https://www.meteor.com/projects)，并研究文档[https://docs.meteor.com](https://docs.meteor.com)。

我希望你有一个伟大的时间阅读这本书，你现在准备开始制作伟大的应用使用Meteor!