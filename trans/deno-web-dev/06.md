...
当然，我们可以在每个请求中单独执行这一操作，但由于这是我们希望在跨应用程序中执行的操作，所以我们将它作为一个中间件添加到`Application`对象中。
...
我们当前的`UserRepository`是负责将用户连接到数据库的模块。 为了使我们的应用程序连接到 MongoDB 实例，而不是内存中的数据库，我们希望更改这个实例。
