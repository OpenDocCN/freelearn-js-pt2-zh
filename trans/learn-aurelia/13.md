...
在本附录中，我们将看到在撰写本文时使用`requirejs`的基于 CLI 的项目与从框架开始的基于Webpack的项目之间的区别。

### 注

本附录的目的不是为了涵盖Webpack本身。因此，我强烈建议您在继续阅读之前，先熟悉一下 Webpack（如果您还没有熟悉的话）。
...
首先，配置 Webpack 可能会很复杂，也很吓人。Webpack骨架使用`easy-webpack`（[https://github.com/easy-webpack/core](https://github.com/easy-webpack/core) 简化此配置过程。使用`easy-webpack`的另一个巨大优势是它强制执行社区标准，并且使重用复杂的配置片段变得非常容易。

因此，您可以使用[提供的许多配置模块中的一个 https://github.com/easy-webpack](https://github.com/easy-webpack) 或其他人，甚至您自己的，以进一步定制Webpack的配置。
