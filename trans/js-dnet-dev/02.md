...
JavaScript正在成为开发大型应用的强大平台，异常处理在处理程序中的异常并在需要时传播异常方面起着重要作用。与 C#或任何其他编程语言一样，JavaScript 提供了`try`、`catch`和`finally`关键字来注释代码以处理错误。JavaScript 提供了使用嵌套的`try catch`语句和条件处理`catch`块中不同条件的相同方法。
